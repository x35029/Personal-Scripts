...
# *****  Change Logging Path and File Name Here  *****
    ...
    $sOutFileName	= "Install-Update.log" # Log File Name
...
Function MainSub
{
    ...
    If ($bRebootState.RebootPending){
        Write-Log -sMessage "Machine $env:computername is pending reboot. Running script again at a later time." -iTabs 1
        Write-Log -sMessage "" -iTabs 0  
        Log-Event -sMessage "$SCRIPT_TITLE ($sScriptName) $SCRIPT_VERSION
    ...
    if(($Install) -and ($Push)){
        ...
        foreach ($kb in $aKBToInstall){
            ...
            $exitWusa = (Start-Process -FilePath "c:\windows\system32\wusa.exe" -ArgumentList "`"$PSScriptRoot`\$kb`"" -PassThru -wait -ErrorAction Stop).ExitCode                                                 
